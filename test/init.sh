...
INFECT_SCRIPT='curl https://raw.githubusercontent.com/elitak/nixos-infect/master/nixos-infect | PROVIDER=hetznercloud NIX_CHANNEL=nixos-23.05 bash 2>&1 | tee /tmp/infect.log'

# Run Nixos Infect
ssh -o "UserKnownHostsFile=./known_hosts" -i ./id_rsa root@"$IP" 'bash -s' < <(echo "$INFECT_SCRIPT")
...
ssh -o "UserKnownHostsFile=./known_hosts" -i ./id_rsa root@"$IP" 'nix-shell -p git --run "nixos-rebuild switch"'
